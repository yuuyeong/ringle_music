  ...
  describe 'POST /api/group_memberships' do
    ...
    context 'when user is owner of the group' do
      ...
      context 'with invalid user_id params' do
        let(:params) { { group_membership: { group_id: group.id, user_id: owner.id } }.to_json }

        it 'returns status code 422' do
          post '/api/group_memberships', params: params, headers: auth_headers
          expect(response).to have_http_status(:unprocessable_entity)
        end
      end

      context 'with invalid group_id params' do
        let(:params) { { group_membership: { group_id: 0, user_id: owner.id } }.to_json }
      ...
      context 'with valid params' do
        let(:params) { { group_membership: { group_id: group.id, user_id: user.id } }.to_json }
    ...
    context 'when user already belongs to the group' do
      ...
      let(:auth_headers) { authenticated_header(user) }
      let(:params) { { group_membership: { group_id: group.id, user_id: member.id } }.to_json }
